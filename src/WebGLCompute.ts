	...
	stepPolyline(
		...
		for (let i = 0; i < vertices.length; i++) {
			...
			if ((i < vertices.length - 2) || closeLoop) {
				...
				n3[0] = n1[0] + n2[0];
				n3[1] = n1[1] + n2[1];
				const length3 = n1[0] * n2[0] + n1[1] * n2[1];
				n3[0] /= length3;
				n3[1] /= length3;
				...
				if (cross < 0) {
					...
					positions[2 * ((index + 2) % (4 * vertices.length))] = v3[0] + n3[0] * offset;
					positions[2 * ((index + 2) % (4 * vertices.length)) + 1] = v3[1] + n3[1] * offset;
				} else {
					positions[2 * index + 2] = v2[0] - n3[0] * offset;
					positions[2 * index + 3] = v2[1] - n3[1] * offset;
					positions[2 * ((index + 2) % (4 * vertices.length)) + 2] = v3[0] - n3[0] * offset;
					positions[2 * ((index + 2) % (4 * vertices.length)) + 3] = v3[1] - n3[1] * offset;
				}
			}
		}
		if (closeLoop) {
