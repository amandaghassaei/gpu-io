...
can render to nextState using currentState as an input.`);
	...
	stepSegment(
		...
		const scaledDiffX = diffX / width;
		const scaledDiffY = diffY / height;
		const scaledLength = Math.sqrt(scaledDiffX * scaledDiffX + scaledDiffY * scaledDiffY);
		program.setUniform('u_internal_length', scaledLength, 'FLOAT');
